...
policy_options = [0, 1, 2, 3]

policy_combinations = list(itertools.product(policy_options, repeat=N))



def optimize_policy(gain_to_cost, expected_gains, expected_costs):

    num_patients = xs.shape[0]
    num_combinations = len(policy_combinations)
    variables = cp.Variable((num_combinations,))
    
    
    constraints = [
        cp.sum(variables) == 1, variables <= 0  # The sum of all variables should be 1 
    ]
    ...
    for i, policy in enumerate(policy_combinations):
        current_gain = np1.zeros((num_patients,))
        current_cost = np1.zeros((num_patients,))
        for j, k in enumerate(policy):
            # Extract the expected gain for the patient j with the policy k
            current_gain[j] = expected_gains[j, k]
            current_cost[j] = expected_costs[j, k]
        
        total_gain += cp.multiply(variables[i], np.sum(current_gain))
        total_cost += cp.multiply(variables[i], np.sum(current_cost))
        
    
    constraints.append( total_cost/num_patients <= 40 )
    ...
    optimal_policy = policy_combinations[np1.argmax(variables.value)]
...
policy = optimize_policy(6, gains, costs)
